 ...
 * 设备管理列表页面
 * */
define(['Tool', 'component', 'text!/tpl/sell-after/strainer.html'], function (Tool, component, strainer) {
    ...
    var Params = {
        ...
        isBind: urlParam.isBind || "",
        isExpire: urlParam.isExpire || "",
    }
    var init = {
        default: function () {
            init.base();
        ...
        getList: function () {
            ...
            Tool.createLoadding();
            $.ajax({
                url: API.sellDeviceStrainerList,
                type: "POST",
                data: data,
                success: function (resp) {
                    if (resp.code == 0) {
            ...
            new Vue({
                ...
                data: {
                    ...
                    titles: [
                        ...
                        {
                            select: true,
                            active: (Params.isBind == "1" && "绑定") || (Params.isBind == "0" && "未绑定") || "是否绑定",
                            options: [
                                {channelName: "是否绑定", class: (Params.isBind == "") && "selected"},
                                {channelName: "绑定", isBind: "1", class: (Params.isBind == "1") && "selected"},
                                {channelName: "未绑定", isBind: "0", class: (Params.isBind == "0") && "selected"}
                            ],
                            click: function (val, list) {
                                Params.isBind = val.isBind;
                                Params.page=0;
                                //改变链接值
                                Tool.changeUrlVal([
                                    {name: "page", val: ""},
                                    {name: "isBind", val: val.isBind}
                                ]);
                                //获取设备型号列表
                                init.getFindByCategory();
                            }
                        ...
                        {
                            select: true,
                            active: (Params.isExpire == "1" && "过期") || (Params.isExpire == "0" && "未过期") || "是否过期",
                            options: [
                                {channelName: "是否过期", class: (Params.isExpire == "") && "selected"},
                                {channelName: "过期", isExpire: "1", class: (Params.isExpire == "1") && "selected"},
                                {channelName: "未过期", isExpire: "0", class: (Params.isExpire == "0") && "selected"}
                            ],
                            click: function (val, list) {
                                Params.isExpire = val.isExpire;
                                Params.page=0;
                                //改变链接值
                                Tool.changeUrlVal([
                                    {name: "page", val: ""},
                                    {name: "isExpire", val: val.isExpire}
                                ]);
                                //获取设备型号列表
                                init.getFindByCategory();
                            }
